/**
 * @file lexer.c
 * This file contains function definitions of the "lexer.h" file.
*/
 ...
 * It deallocates the tokens generated by \a mr_lexer_match function. \n
 ...
 * @param cnn
 * Connector of \a data and its field (`.` or `->`).
*/
#define mr_lexer_tokens_free(cnn)                          \
    do                                                     \
    {                                                      \
        while (data cnn size--)                            \
            mr_free(data cnn tokens[data cnn size].value); \
        mr_free(data cnn tokens);                          \
    } while (0)
 ...
 * This macro should only be called from the \a mr_lexer_match or \a mr_lexer_generate functions.
 * @param typ
 * Type of the token that needs to be created.
 * @param inc
 * Length of the token in characters (to increment the <em>pos</em>).
*/
...
/**
 * @def mr_lexer_realloc(size)
 * It reallocates the value block (the \a value field of the \a token structure). \n
...
#define mr_lexer_tokens_realloc                                                                    \
    ...
    {                                                                                              \
        ...
        {                                                                                          \
            block = mr_realloc(data->tokens, (data->alloc += data->exalloc) * sizeof(mr_token_t)); \
            if (!block)                                                                            \
            {                                                                                      \
                data->flag = MR_LEXER_MATCH_FLAG_MEMORY;                                           \
                return;                                                                            \
    ...
    {                                                                \
        ...
        {                                                            \
            ...
            data->pos.ln++;                                          \
                                                                     \
        if (token->size == alloc)                                    \
            mr_lexer_realloc((alloc += exalloc) * sizeof(mr_chr_t)); \
                                                                     \
...
void mr_lexer_generate_identifier(mr_lexer_match_t *data)
{
    ...
    {
        if (token->size == alloc)
            mr_lexer_realloc((alloc += MR_LEXER_IDENTIFIER_SIZE) * sizeof(mr_chr_t));
...
void mr_lexer_generate_number(mr_lexer_match_t *data)
{
    ...
    do
    {
        ...
        if (token->size == alloc)
            mr_lexer_realloc((alloc += MR_LEXER_NUMBER_SIZE) * sizeof(mr_chr_t));
    ...
    {
        if (token->size == alloc)
            mr_lexer_realloc((alloc += MR_LEXER_NUMBER_EXP_SIZE) * sizeof(mr_chr_t));
